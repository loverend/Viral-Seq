...
get_qc <- function(outputdir, type_run, infection=NA){
	...
	df$CMV <- NA 
	if(!is.na(infection)){
		df$CMV[df$Sample %like% "CMV"] <- "POSITIVE"
		df$CMV[!df$Sample %like% "CMV"] <- "NEGATIVE"
	}
	...
	for (i in 1:length(unique(df$Virus))){
		...
		p1 <- ggplot(QC_result, aes(color=CMV, x=N_reads, y=(N_unique_reads))) + geom_point()  + theme_classic() + xlab("Number of Mapped Reads") + ylab("Number Uniquely Mapped Reads")  + ggtitle(name_virus)+ expand_limits(x = 0, y = 0)+ guides(color="none")
		p2 <- ggplot(QC_result, aes(color=CMV, x=N_unique_reads, y=(Spatial_distribution*100))) + geom_point()  + theme_classic() + xlab("Number of Uniquely Mapped Reads") + ylab("% Mapped genome") + ylim(0, 100) + ggtitle(name_virus)+ guides(color="none")
		p3 <- ggplot(QC_result, aes(color=CMV, x=Sequence_entropy, y=(Spatial_distribution*100))) + geom_point()  + theme_classic() + xlab("Sequence Entropy") + ylab("% Mapped genome")  + ylim(0, 100) + ggtitle(name_virus)+ guides(color="none")
		p4 <- ggplot(QC_result, aes(color=CMV, x=Longest_contig, y=DUST_score)) + geom_point()  + theme_classic() + xlab("Longest Contig (nt)") + ylab("DUST Score") + ggtitle(name_virus)+ guides(color="none")
		p5 <- ggplot(QC_result, aes(color=CMV, x=Mean_read_quality, y=Sd_read_quality)) + geom_point()  + theme_classic() + xlab("Mean Read Quality") + ylab("SD Read Quality") + ggtitle(name_virus)+ guides(color="none")
		p6 <- ggplot(QC_result, aes(color=CMV, x=Longest_contig, y=Coverage_per_read_Ratio)) + geom_point()  + theme_classic() + xlab("Longest Contig") + ylab("Coverage per Read Ratio") + ggtitle(name_virus) + guides(color="none") +geom_vline(xintercept=100, col="blue") +geom_vline(xintercept=200, col="green")
		p7 <- ggplot(QC_result, aes(color=CMV, x=(Spatial_distribution*100), y=Coverage_per_read_Ratio)) + geom_point() + theme_classic() + xlab("% Mapped genome") + ylab("Coverage per Read Ratio") + ggtitle(name_virus) + xlim(0, 100) + guides(color="none")
		p8 <- ggplot(QC_result, aes(color=CMV, y=Coverage_Percent_1000bp, x=N_unique_reads)) + geom_point()  + theme_classic() + ylab("% Coverage for 1000bp with highest coverage") + xlab("Number Uniquely Mapped Reads") + ggtitle(name_virus)+ guides(color="none")
		if(length(rownames(QC_result))>=1){
			...
			if(length(rownames(QC_result))>1){
				...
				cols_discard2 <- lapply(lapply(QC_result, is.na), sum)
