...
option_list <- list( 
  ...
  make_option(c("-o", "--outputdir"), action="store", default='/gpfs2/well/immune-rep/shared/CODE/Viral-Seq/ExampleOutput', type="character", help="Path to output directory"),
  ...
  make_option(c("-f", "--fastq"), action="store", type="character", default = '/well/immune-rep/shared/CODE/Viral-Seq/SingleCell/Downsamples/sub100k.fa', help="Path to input FASTQ file [default]"),
  make_option(c("-r", "--runname"), action="store", type="character", default="Single_Cell", help="Run Name [default]"),
...
cat("SingleCell_VirSeq_UniqueMapping by Lauren Overend. \n", file=log, append=TRUE)
  ...
  for(i in 1:length(rownames(temp_chromosome_count))) {
	...
	if(length(BAM_file_2)==0){
		...
		if(length(rownames(temp_chromosome_count))==1){
              ...
              Mean_dust_score=numeric(),Percent_high_quality_reads=numeric(), length_per_read=numeric(), read_length=numeric(), coverage_per_thousand=numeric())
		}
		next
	} else {
	...
	if(length(BAM_file) >0){
		...
		## Find the most densley covered part of the genome and calculate how coverd this is (100 is about 10 read pairs). 
		if(length(Covered_genome)>1000){
			coverage_per_thousand <- 0
			for(c in 1:length(Covered_genome)){
				if((c+999) > length(Covered_genome)){
					break
				}
				subset_coverage <- Covered_genome[c:(c+999)]
				coverage_percent_100 <- (length(subset_coverage[subset_coverage>0])/1000)*100
				if(coverage_percent_100 > coverage_per_thousand){
					coverage_per_thousand <- coverage_percent_100
				}
			}
		} else {
		coverage_per_thousand <- "NA"
		}
		...
		plot(Covered_genome, type = "l", xlab=paste0(z, " Genome (bp)"), ylab="No. Mapped Reads)")
		dev.off()
    ...
    QC_temp = c(N_mapped_reads,N_unique_mapped_reads,Percent_uniquely_mapped,
                ...
                Mean_dust_score,Percent_high_quality_reads, length_per_read, read_length, coverage_per_thousand)
...
if (length(QC_result[,1])>0){
	colnames(QC_result) = c("N_reads","N_unique_reads","Percent_uniquely_mapped","Mean_read_quality","Sd_read_quality",c("A","C","G","T"),"Sequence_entropy","Spatial_distribution","Longest_contig","DUST_score","Percent_high_quality_reads", "Coverage_per_read_Ratio", "Mean_Read_Length", "Coverage_Percent_1000bp")
...
if(length(rownames(QC_result))>0){
	if(sum(QC_result$N_unique_reads)>0){
	  p1 <- ggplot(QC_result, aes(shape=PassedFiltering, color=genome, x=N_reads, y=(N_unique_reads))) + geom_point() + scale_color_discrete(drop=FALSE) + scale_shape(drop=FALSE) + labs(color="Genome", shape="Passed Filtering") + theme_classic() + xlab("Number of Mapped Reads") + ylab("Number Uniquely Mapped Reads") + geom_vline(xintercept=(opt$t), linetype="dashed", color="blue") + ggtitle("Viral Summary Unique Reads: Multimapping Read Count")+ expand_limits(x = 0, y = 0)
	  p2 <- ggplot(QC_result, aes(shape=PassedFiltering, color=genome, x=N_unique_reads, y=(Spatial_distribution*100))) + geom_point() + scale_color_discrete(drop=FALSE) + scale_shape(drop=FALSE) + labs(color="Genome", shape="Passed Filtering") + theme_classic() + xlab("Number of Uniquely Mapped Reads") + ylab("% Mapped genome") + geom_hline(yintercept=5, linetype="dashed", color="blue") + ylim(0, 100) + ggtitle("Viral Summary Unique Reads: Unique Read Count")
	  p3 <- ggplot(QC_result, aes(shape=PassedFiltering, color=genome, x=Sequence_entropy, y=(Spatial_distribution*100))) + geom_point() + scale_color_discrete(drop=FALSE) + scale_shape(drop=FALSE) + labs(color="Genome", shape="Passed Filtering") + theme_classic() + xlab("Sequence Entropy") + ylab("% Mapped genome") + geom_hline(yintercept=5, linetype="dashed", color="blue") + ylim(0, 100) + geom_vline(xintercept=1.2, linetype="dashed", color="blue") + ggtitle("Viral Summary Unique Reads: Unique Read Sequence Complexity")
	  p4 <- ggplot(QC_result, aes(shape=PassedFiltering, color=genome, x=Longest_contig, y=DUST_score)) + geom_point() + scale_color_discrete(drop=FALSE) + scale_shape(drop=FALSE) + labs(color="Genome", shape="Passed Filtering") + theme_classic() + xlab("Longest Contig (nt)") + ylab("DUST Score") + geom_vline(xintercept=(2*Mean_mapping_length), linetype="dashed", color="grey") + geom_vline(xintercept=(3*Mean_mapping_length), linetype="dashed", color="green") + ggtitle("Viral Summary Unique Reads: DUST Score")
	  ...
	  p8 <- ggplot(QC_result, aes(shape=PassedFiltering, color=genome, y=Coverage_Percent_1000bp, x=N_unique_reads)) + geom_point() + scale_color_discrete(drop=FALSE) + scale_shape(drop=FALSE) + labs(color="Genome", shape="Passed Filtering") + theme_classic() + ylab("% Coverage for 1000bp with highest coverage") + xlab("Number Uniquely Mapped Reads") + ggtitle("Viral Summary Unique Reads: Read Coverage") + geom_vline(xintercept=0.5, linetype="dashed", color="red") + geom_hline(yintercept=(2*Mean_mapping_length)/10, linetype="dashed", color="grey") + geom_hline(yintercept=(3*Mean_mapping_length)/10, linetype="dashed", color="green")
...
featurecommand = paste("featureCounts -t transcript -g gene_id -R BAM -T ", N_thread, " -a ", path_to_gtf, " -o ", temp_output_dir, "/counts.txt -s 0 " , feature_count_path, " 2> ", name_prefix, "_FEATURE_COUNTS.log", sep="")
...
command_sort =paste("samtools sort ",paste0(feature_count_path, ".featureCounts.bam"), " -o ", temp_output_dir,"/Assigned_sorted.bam",sep = "")
system(command_sort)
cat("Done.  \n", file=log, append=TRUE)
...
viral_cols <- grep("NC_", colnames(expression_table), value=TRUE)
...
if(length(viral_cols)>= 1){
  ...
  for (i in viral_cols){
    name=paste0("percent_viral_reads_", i)
    virus <- i
